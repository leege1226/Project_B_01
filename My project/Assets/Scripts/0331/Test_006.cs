...
public class Test_006 : MonoBehaviour
{
    ...
    public int[] array = new int[5];                    //배열 5개 공간을 선언하고 이름을 array
    ...
    void Start()
    {
        ...
        for(int i = 0; i < 5; i++)                        //for문을 통해 console창에 모든 array 배열 안에 있는 0~4까지 내용을 표시
        {
            Debug.Log(array[i]);
        }
        ...
        {
            if(points[i] > 90)                              //points[i] 각각 for을 돌면서 비교해서 90이 넘는 숫자만 프린트한다.
            {
                Debug.Log(points[i]);
            }
        }

        int sum = 0;                                        //합 값을 구하기 위해서 임시적으로 sum변수 선언
